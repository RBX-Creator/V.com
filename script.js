...
function renderDmContactList(filter = "") {
  ...
  const match = (u) => {
    if (!q) return true;
    if (u.isOrderChat && "bestellung".includes(q)) return true;
...
function openChat(partnerUid, partnerProfile) {
  ...
  if (nameEl) {
    nameEl.textContent = isOrder
      ? ("Bestellung · @" + (partnerProfile.handle || "user"))
  ...
  dmUnsub = onSnapshot(q2, (snap) => {
    ...
    if (!activeChatPartner.isOrderChat && msgs.some(m => m.shopOrder)) {
      ...
      if (nameEl) nameEl.textContent = "Bestellung · @" + (partnerProfile.handle || "user");
    ...
    return `<div class="shop-item" data-id="${p.id}">
      ...
      <div class="shop-item-body">
        ...
        <div class="shop-item-btns">
          <button class="shop-btn-cart mini" data-cart="${p.id}" ${stockNum < 1 ? "disabled" : ""}>In den Warenkorb</button>
          <button class="shop-btn-buy mini" data-buy="${p.id}" ${stockNum < 1 ? "disabled" : ""}>Per Nachricht kaufen</button>
        </div>
      </div>
    </div>`;
  }).join("");
...
function openCart() {
  ...
  if (!cart.length) {
    items.innerHTML = `<div class="dm-empty"><p>Warenkorb ist leer</p></div>`;
  } else {
  ...
  if (totalEl) totalEl.textContent = "Gesamt: " + formatPrice(getCartTotal());
...
async function startShopOrderFromCart() {
  ...
  if (!owner) return alert("Verkäufer (@vida) nicht gefunden");
  ...
  const paypal = owner.paypalName || "PayPal-Name schreibt der Verkäufer gleich";
  const text =
    `🛒 KAUFANFRAGE (V Shop)\nBestell-Nr: ${orderRef.id.slice(0, 8)}\n\n` +
    `${lines}\n\nGesamt: €${total.toFixed(2)}\n\n---\n` +
    `Bitte PayPal an: ${paypal}\n` +
    `Danach hier Lieferadresse + optional Screenshot der Zahlung schicken.\n` +
  ...
  await setDoc(doc(db, "chats", key), {
    ...
    sellerUid: owner.uid,
    closed: false,
    lastText: "🛒 Kaufanfrage · €" + total.toFixed(2),
  ...
  alert("Chat mit dem Verkäufer geöffnet.\nDer Chat heißt „Bestellung“ und steht oben in deinen DMs.");
}
...
document.getElementById("cartCheckout")?.addEventListener("click", startShopOrderFromCart);
