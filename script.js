...
function loadShop() {
  ...
  if (shopUnsub) return; // already listening
...
function renderShop() {
  ...
  let list = allProducts.filter(p => {
    if (shopCat !== "all" && (p.category || "other") !== shopCat) return false;
    if (!term) return true;
  ...
  grid.innerHTML = list.map(p => `
    <div class="shop-card" data-id="${p.id}">
      <img class="shop-card-img" src="${p.imageUrl || ""}" alt="" loading="lazy" onerror="this.style.background='#202327'" />
      <div class="shop-card-body">
        <div class="shop-card-name">${escapeHtml(p.name || "Item")}</div>
        <div class="shop-card-price">${formatPrice(p.price)}</div>
        <div class="shop-card-cat">${escapeHtml(p.category || "other")}</div>
      </div>
    </div>
  `).join("");
...
function openProduct(id) {
  ...
  document.getElementById("pmName").textContent = p.name || "Product";
  document.getElementById("pmPrice").textContent = formatPrice(p.price);
  document.getElementById("pmDesc").textContent = p.description || "";
  document.getElementById("pmSeller").textContent = p.sellerHandle ? ("Seller: @" + p.sellerHandle) : "";
  const del = document.getElementById("pmDeleteBtn");
  if (del) {
    if (isOwner() || (currentUser && p.sellerId === currentUser.uid)) del.classList.remove("hidden");
...
document.getElementById("pmBuyBtn")?.addEventListener("click", () => {
  ...
  if (activeProduct.link) {
    window.open(activeProduct.link, "_blank", "noopener");
  } else if (activeProduct.sellerHandle) {
    alert("Contact seller @" + activeProduct.sellerHandle + " via Messages to buy.");
    switchPage("messages");
  } else {
    alert("No buy link set. Contact the seller.");
  }
});

document.getElementById("pmDeleteBtn")?.addEventListener("click", async () => {
  if (!activeProduct || !currentUser) return;
  if (!isOwner() && activeProduct.sellerId !== currentUser.uid) return;
...
document.getElementById("prodImage")?.addEventListener("change", async (e) => {
  ...
  if (file.size > 1.5 * 1024 * 1024) {
    alert("Image max 1.5MB");
    return;
  }
  pendingProdImage = await readFileAsDataURL(file);
  document.getElementById("prodImagePreview").innerHTML = `<img src="${pendingProdImage}" alt="" style="width:100%;max-height:160px;object-fit:cover;border-radius:8px" />`;
});
...
document.getElementById("prodSaveBtn")?.addEventListener("click", async () => {
  ...
  const category = document.getElementById("prodCat")?.value || "other";
  const description = document.getElementById("prodDesc")?.value.trim() || "";
  const link = document.getElementById("prodLink")?.value.trim() || "";
  if (!name || !price) return alert("Name and price required");
  if (!pendingProdImage) return alert("Please add an image");
  await addDoc(collection(db, "products"), {
    ...
    description,
    link,
    imageUrl: pendingProdImage,
    sellerId: currentUser.uid,
    sellerHandle: currentProfile?.handle || "seller",
    sellerName: currentProfile?.name || "Seller",
    createdAt: serverTimestamp()
  });
