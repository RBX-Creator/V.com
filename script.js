...
authSubmit.addEventListener("click", async () => {
  ...
  try {
    if (isRegisterMode) {
      ...
      alert("Bestätigungs-E-Mail wurde gesendet. Bitte öffne dein Postfach und bestätige die E-Mail, bevor du posten oder liken kannst.");
    } else {
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    // seed disabled – no auto demo posts
    loadPosts();
      ...
      if (!ban) {
        ...
        document.getElementById("resendVerify")?.addEventListener("click", async () => {
          try {
            ...
            alert("E-Mail erneut gesendet. Bitte Postfach prüfen (auch Spam).");
...
async function loadReplies(postId) {
  ...
  // Primary: replies stored on the post document (no extra index / rules needed)
...
function renderPosts(posts, containerId) {
  ...
  timeline.querySelectorAll(".reply-send").forEach(btn => {
    btn.addEventListener("click", async (e) => {
      ...
      const post = allPosts.find(p => p.id === postId);
      const prev = (post && post.replyList) ? post.replyList : [];
      const replyList = [...prev, replyObj];
      await updateDoc(doc(db, "posts", postId), {
        replyList,
        repliesCount: replyList.length
      });
