  ...
  listEl.innerHTML = all.map(u => {
    if (u.isOrderChat) {
      return `<div class="dm-item dm-order-item" data-uid="${u.uid}" data-order="1">
        ...
        <button type="button" class="dm-hide-btn" data-hide="${u.uid}" data-order="1" title="Delete chat for you">🗑</button>
      </div>`;
    }
    return `<div class="dm-item" data-uid="${u.uid}">
      ...
      <button type="button" class="dm-hide-btn" data-hide="${u.uid}" data-order="0" title="Delete chat for you">🗑</button>
    </div>`;
  }).join("");
...
document.querySelectorAll(".legal-link").forEach(el => {
  el.addEventListener("click", (e) => {
    e.preventDefault();
    openLegal(el.dataset.legal || "privacy");
  });
});
document.querySelectorAll(".legal-tab").forEach(tab => {
  tab.addEventListener("click", () => openLegal(tab.dataset.legalTab));
});
...
document.getElementById("legalModal")?.addEventListener("click", (e) => {
  if (e.target.id === "legalModal") e.target.classList.add("hidden");
});
