...
function loadPosts() {
  ...
  onSnapshot(q, (snapshot) => {
    ...
    // max 200 newest posts (performance)
    if (allPosts.length > MAX_POSTS) allPosts = allPosts.slice(0, MAX_POSTS);
...
const ICO = {
  ...
  like: `<svg class="ico" viewBox="0 0 24 24" aria-hidden="true"><path d="M16.697 5.5c-1.222-.06-2.679.51-3.89 2.16l-.805 1.09-.806-1.09C9.984 6.01 8.526 5.44 7.304 5.5c-1.243.07-2.349.78-2.91 1.91-.552 1.12-.523 2.58.436 4.19.96 1.61 2.844 3.46 5.766 5.71l.502.37.503-.37c2.922-2.25 4.807-4.1 5.766-5.71.959-1.61.988-3.07.436-4.19-.561-1.13-1.667-1.84-2.91-1.91zm4.187 11.572l.002.003.002.002.003.002.002.001.002.002.003.001.002.001.002.001.002.001.002.001.001.001.002.001.001.001.002.001.001.001.001.001.001h.001l.001.001h.001c0 .001 0 .001 0 .001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001h.001v.001H12l-.001-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001h-.001v-.001l-.001-.001v-.001h-.001l-.001-.001v-.001h-.001l-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001-.001L12 17.072l-4.884 3.812c-.001.001-.001.001-.001.001l-.001.001-.001.001-.001.001-.001.001-.001.001-.001.001-.001.001-.001.001-.001.001-.001.001-.001.001-.001.001h-.001l-.001.001h-.001l-.001.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001h-.001v.001H7.3c-2.922-2.25-4.807-4.1-5.766-5.71-.96-1.61-.988-3.07-.436-4.19.561-1.13 1.667-1.84 2.91-1.91 1.222-.06 2.68.51 3.89 2.16l.805 1.09.806-1.09c1.21-1.65 2.668-2.22 3.89-2.16 1.243.07 2.349.78 2.91 1.91.552 1.12.523 2.58-.436 4.19z"/></svg>`,
  likeOn: `<svg class="ico" viewBox="0 0 24 24" aria-hidden="true"><path d="M20.884 13.19c-1.175 2.21-3.12 3.96-5.185 5.57l-3.699 2.89-3.699-2.89c-2.065-1.61-4.01-3.36-5.185-5.57C1.55 10.24 1 8.242 1 6.5 1 3.462 3.462 1 6.5 1c1.797 0 3.45.932 4.5 2.345C12.05 1.932 13.703 1 15.5 1 18.538 1 21 3.462 21 6.5c0 1.742-.55 3.74-2.116 6.69z"/></svg>`,
  ...
  posts.forEach(post => {
    ...
    el.innerHTML = `
      ...
      <div class="post-content">
        ...
        <div class="replies-section<div class="replies-section ${openReplyId === post.id ? "" : "hidden"}" data-replies-for="${post.id}">
...
document.getElementById("editProfileBtn")?.addEventListener("click", () => {
  ...
  if (av) av.innerHTML = currentProfile?.avatarUrl
    ? `<img src="${currentProfile.avatarUrl}" alt="" /><p class="settings-hint">Tap “Profile picture” to change</p>` : "";
...
async function loadDmList() {
  ...
  try {
    ...
    for (const d of snap.docs) {
      ...
      // fully closed by owner → gone for everyone
    ...
    items.innerHTML = cart.map((c, i) => `
      <div class="cart-row">
        ...
        <div class="cart-row-actions">
          ...
          <button data-rm="${i}">${ICO.trash}</button>
        </div>
      </div>`).join("");
...
const LEGAL = {
  ...
  terms: `
    ...
    <h3>4. Shop / resale</h3>
    <p>Shop purchases are arranged via <strong>PayPal DM</strong> with the seller. V.com is not an automated payment provider. Shipping is as agreed in chat (e.g. Germany only if stated). Resolve payment and delivery disputes with the seller; the owner moderates abuse.</p>
    <h3>5. Moderation</h3>
