...
// Splash is driven by intro video (ended) + failsafe in index.html.
// Only ensure cart isn't blocking after a short delay.
setTimeout(() => {
  try {
    const cp = document.getElementById("cartPanel");
    if (cp) { cp.classList.add("hidden"); cp.style.display = "none"; }
    // if video missing / already ended, hide splash
    const v = document.getElementById("splashVideo");
    if (!v || v.error || (v.readyState >= 2 && v.ended)) {
