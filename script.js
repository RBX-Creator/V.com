...
/** Compress image dataURL for upload (max edge px, jpeg quality). */
function compressDataUrl(dataUrl, maxEdge = 1200, quality = 0.72) {
  return new Promise((resolve, reject) => {
...
/** Upload dataURL to Firebase Storage, return download URL. */
async function uploadProductImage(dataUrl, productKey, index) {
  if (!dataUrl) return "";
  // already a remote URL
  if (/^https?:\/\//i.test(dataUrl)) return dataUrl;
  const compressed = await compressDataUrl(dataUrl, 1200, 0.72);
  const path = `products/${productKey}/${index}_${Date.now()}.jpg`;
...
function renderShop() {
  ...
  list.innerHTML = items.map(p => {
    ...
    const brandBadge = brand ? `<span class="shop-brand-badge">${escapeHtml(brand)}</span>` : "";
...
function openProduct(id) {
  ...
  if (pmBrand) {
    ...
    if (b) { pmBrand.textContent = b; pmBrand.classList.remove("hidden"); }
    else { pmBrand.textContent = ""; pmBrand.classList.add("hidden"); }
  }
...
document.getElementById("pmEditBtn")?.addEventListener("click", () => {
  ...
  const brandEl = document.getElementById("prodBrand"); if (brandEl) brandEl.value = p.brand || "";
...
document.getElementById("shopAddBtn")?.addEventListener("click", () => {
  ...
  const _br = document.getElementById("prodBrand"); if (_br) _br.value = "";
...
document.getElementById("prodSaveBtn")?.addEventListener("click", async () => {
  if (!currentUser || !isOwner()) return alert("Owner only");
  const name = document.getElementById("prodName")?.value.trim();
  const brand = document.getElementById("prodBrand")?.value.trim().slice(0, 40) || "";
  ...
  if (!price || isNaN(parseFloat(price))) return alert("Price required");
  if (stock === "" || isNaN(parseInt(stock, 10))) return alert("Stock required");
  if (!buyLink) return alert("Buy link required");
  if (!/^https?:\/\//i.test(buyLink)) buyLink = "https://" + buyLink;
  if (!pendingProdImages.length && !pendingProdImage && !editId) return alert("Image required");

  const btn = document.getElementById("prodSaveBtn");
  const oldLabel = btn?.textContent || "Save";
  if (btn) { btn.disabled = true; btn.textContent = "Uploading…"; }

  try {
    const rating = Math.max(1, Math.min(5, parseInt(document.getElementById("prodRating")?.value || "5", 10) || 5));
    const productKey = editId || ("new_" + Date.now().toString(36));
    let imgs = (pendingProdImages.length ? pendingProdImages : (pendingProdImage ? [pendingProdImage] : [])).slice(0, 5);

    // Upload any data-URL images to Firebase Storage (Firestore docs max ~1MB — base64 breaks)
    const uploaded = [];
    for (let i = 0; i < imgs.length; i++) {
      ...
      if (/^https?:\/\//i.test(src)) {
        ...
        if (btn) btn.textContent = "Uploading image " + (i + 1) + "/" + imgs.length + "…";
        const url = await uploadProductImage(src, productKey, i);
        uploaded.push(url);
      }
    }

    // edit without new images: keep existing remote images
    if (!uploaded.length && editId) {
      const existing = allProducts.find(p => p.id === editId);
      if (existing) {
        const keep = productImages(existing);
        uploaded.push(...keep);
      }
    }
    ...
    pendingProdImage = null;
    pendingProdImages = [];
    const prev = document.getElementById("prodImagesPreview");
    if (prev) prev.innerHTML = "";
    const mainPrev = document.getElementById("prodImagePreview");
    if (mainPrev) mainPrev.innerHTML = "";
  } catch (err) {
    console.error(err);
    const msg = err?.message || String(err);
    if (/storage|unauthorized|permission/i.test(msg)) {
      alert("Image upload blocked. In Firebase Console → Storage → Rules, allow write for authenticated users.\n\n" + msg);
    } else if (/exceeds|too large|1 MiB|size/i.test(msg)) {
      alert("Product data too large. Images must go to Storage (not Firestore).\n\n" + msg);
    } else {
