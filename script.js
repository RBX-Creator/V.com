...
let cart = []; // {id, name, price, imageUrl, qty, stock}
    ...
    return `<div class="shop-item" data-id="${p.id}">
      ...
      <div class="shop-item-body">
        ...
        <div class="shop-item-btns">
          ...
          <button class="shop-btn-buy mini" data-buy="${p.id}" ${stockNum < 1 ? "disabled" : ""}>Jetzt kaufen</button>
        </div>
      </div>
    </div>`;
  }).join("");
...
function addToCart(id) {
  ...
  if (stock < 1) {
    alert("Nicht auf Lager");
    return;
  }
  const existing = cart.find(c => c.id === id);
  if (existing) {
    if (existing.qty >= stock) {
      alert("Max. verfügbare Menge erreicht");
      return;
    }
    existing.qty += 1;
  } else {
...
function openCart() {
  ...
  document.getElementById("cartTotal").textContent = "Gesamt: " + formatPrice(total);
...
document.getElementById("cartCheckout")?.addEventListener("click", async () => {
  ...
  try {
    for (const c of cart) {
      ...
      if (!p) continue;
      const stock = parseInt(p.stock, 10) || 0;
      const newStock = Math.max(0, stock - c.qty);
      await updateDoc(doc(db, "products", c.id), { stock: newStock });
    }
    alert("Bestellung aufgenommen! Gesamt: " + formatPrice(cart.reduce((s, c) => s + c.price * c.qty, 0)) + "\n\n(Demo: Zahlung folgt später)");
    cart = [];
    updateCartBadge();
    document.getElementById("cartPanel")?.classList.add("hidden");
  } catch (e) {
