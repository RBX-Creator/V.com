...
async function closeOrderChat() {
  ...
  if (isSupport) {
    ...
    if (!confirm("Report/bug-Chat für beide schließen?")) return;
  } else if (isOrder) {
  ...
  if (isSupport) {
    ...
    alert(isSupport ? "Report/bug-Chat geschlossen." : "Order chat closed.");
}
...
function openExternalBuyLink(url) {
  url = (url || "").trim();
  if (!url) {
    alert("Kein Buy-Link für diesen Artikel hinterlegt.");
    return false;
  }
    ...
    items.innerHTML = cart.map((c, i) => `
      <div class="cart-row">
        ...
        <div class="cart-row-actions cart-row-actions-wide">
          <button type="button" class="cart-buy-btn" data-buy="${i}">Kaufen</button>
...
document.getElementById("pmBuyBtn")?.addEventListener("click", () => {
  ...
  if (stock < 1) return alert("Nicht auf Lager");
  if (!activeProduct.buyLink) return alert("Kein Buy-Link für diesen Artikel.");
...
document.getElementById("supportSubmitBtn")?.addEventListener("click", async () => {
  ...
  if (!title) {
    if (statusEl) statusEl.textContent = "Titel fehlt (max. 10 Zeichen).";
    return alert("Titel fehlt (max. 10 Zeichen)");
  }
  const owner = await findUserByHandle("vida");
  if (!owner) return alert("Support derzeit nicht erreichbar (@vida fehlt)");

  const btn = document.getElementById("supportSubmitBtn");
  if (btn) btn.disabled = true;
  try {
    const key = chatKey(currentUser.uid, owner.uid);
    // unique support thread key so multiple tickets don't collide forever
    ...
    // open chat with owner as support thread
    openChat(owner.uid, {
      ...owner,
      uid: owner.uid,
      isSupportChat: true,
    ...
    if (statusEl) statusEl.textContent = "Gesendet — Chat mit Support ist geöffnet.";
    switchPage("messages");
  } catch (e) {
    alert(e.message || String(e));
...
async function loadSupportTickets() {
  ...
  el.innerHTML = `<div class="settings-hint">Laden…</div>`;
  try {
    ...
    if (!tickets.length) {
      el.innerHTML = `<div class="settings-hint">Keine offenen Report/bug-Anfragen.</div>`;
      return;
    }
